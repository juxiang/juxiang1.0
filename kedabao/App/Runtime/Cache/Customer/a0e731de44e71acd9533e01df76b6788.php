	...
	<div class="panel-group" id="accordion">
		...
		<div id="content" class="col-sm-8">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h4 class="panel-title"><i class="glyphicon glyphicon-header"></i> 产品列表</h4>
				</div>
...
<?php if(is_array($dishes)): $i = 0; $__LIST__ = $dishes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><form class="form-horizontal" action="<?php echo U(GROUP_NAME.'/Dishes/update',array('did'=>$vo['id']));?>" method='post' enctype='multipart/form-data'>
	<tr>
		<!-- <td ><input type='text' name='wx_cats' value='<?php echo ($vo["wx_cats"]); ?>' disabled></td> -->
		<td ><input type='text' name='cats' value='<?php echo ($vo["cname"]); ?>' disabled></td>
		<td ><input type='text' name='dname' value='<?php echo ($vo["dname"]); ?>'></td>
		<td ><?php if($vo['isset'] == 1): ?><a href="<?php echo U($vo['url']);?>" target="_blank">预览</a>
			<?php else: ?> <a href="<?php echo U(GROUP_NAME.'/Product/add2',array('did'=>$vo['id']));?>" ><font color="red">设置</font></a><?php endif; ?></td>
		<td ><img src="__ROOT__<?php echo ($vo['picpath']); ?>" height=50 width=50 /></td>
		<td>
		<div class="btn-group btn-group-xs">
			<input type='submit' value='修改' class="btn btn-warning"/>
			<a href="<?php echo U(GROUP_NAME.'/Dishes/delete',array('did'=>$vo['id']));?>" class="btn btn-danger"><i class="glyphicon glyphicon-remove"></i></a>
		</div>
		</td>
	</tr>
