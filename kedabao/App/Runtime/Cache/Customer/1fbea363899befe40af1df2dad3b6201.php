  ...
  $(function(){
    ...
    function getwxcats()
    {
	 ...
	 //alert(json);
      $.each(json, function(i, item) {
		  if(item[0]==$("#company").val())
			 $("<option></option>").val(item[0]).text(item[0]).appendTo($("#wxcats")).attr("selected", true);
		//else
    ...
    function getcats()
    {
	 ...
	 var json=eval('('+decodeURIComponent(data)+')');
      $.each(json, function(i, item) {
		  if(item[1]==$("#did").val())
			$("<option></option>").val(item[1]).text(item[0]).appendTo($("#cats")).attr("selected", true);
		  //else
	...
	function getproducts()
    {
	 ...
	 var json=eval('('+decodeURIComponent(data)+')');
      $.each(json, function(i, item) {
		  if(item[0]==$("#pid").val())
			$("<option></option>").val(item[0]).text(item[2]).appendTo($("#products")).attr("selected", true);
		  //if(item[3]==0)
...
<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div id="content" class="col-sm-8">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h4 class="panel-title"><i class="glyphicon glyphicon-header"></i> 详细产品修改--产品编号：<?php echo ($vo["pid"]); ?></h4>
				</div>

				<div class="panel-body">

<form class="form-horizontal" action="<?php echo U(GROUP_NAME.'/Product/doupdate',array('id'=>$vo['id']));?>" method='post' enctype='multipart/form-data'>
<!--    <div class="form-group">
    <label for="logo" class="col-sm-2 control-label">公司LOGO</label>
    <div class="col-sm-10">
      <input type="file" name="logo" id='logo'>
	  <img src='__ROOT__<?php echo ($vo["logo"]); ?>' height='60' width='60'>
    </div>
  </div> -->
    <div class="form-group">
    <label for="wxcats" class="col-sm-2 control-label">微信二级菜单：</label>
    <div class="col-sm-10">
		<input type='hidden' id='company' value="<?php echo ($vo["company"]); ?>" />
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
		<input type='hidden' id='did' value="<?php echo ($vo["did"]); ?>">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
		<input id='11' type='file' name="picone" >
		<img height='60' width='60' name='picone' src="__ROOT__<?php echo ($vo["picone"]); ?>"></img>
		<small>图片最佳尺寸：80*80，小于100kb</small>
    </div>
  </div>
  <div class="form-group">
    <label for="12" class="col-sm-2 control-label">展示图片2：</label>
    <div class="col-sm-10">
		<input id='12' type='file' name="pictwo" >
		<img height='60' width='60' name='pictwo' src="__ROOT__<?php echo ($vo["pictwo"]); ?>"></img>
		<small>图片最佳尺寸：80*80，小于100kb</small>
    </div>
  </div>
  <div class="form-group">
    ...
    <div class="col-sm-10">
		<input id='13' type='file' name="picthree" >
		<img height='60' width='60' name='picthree' src="__ROOT__<?php echo ($vo["picthree"]); ?>"></img>
		<small>图片最佳尺寸：80*80，小于100kb</small>
    </div>
  </div>
    <div class="form-group">
    ...
    <div class="col-sm-10">
      <textarea id='introduce' name="introduce" placeholder=""><?php echo ($vo["introduce"]); ?></textarea>
    </div>
  </div>
    <div class="form-group">
    <label for="table" class="col-sm-2 control-label">产品性能表:</label>
    <div class="col-sm-10">
      <textarea id='table' type="text" name="table"><?php echo ($vo["table"]); ?></textarea>
    </div>
  </div>
    <div class="form-group">
    <label for="5" class="col-sm-2 control-label">产品性能说明:</label>
    <div class="col-sm-10">
      <textarea id='5' name="capability" cols='90'><?php echo ($vo["capability"]); ?></textarea>
    </div>
    </div>
   <div class="form-group">
